...
#endif
// FOr using A0 on the board as reset line for TinierScreen
#define A0 ANSELBbits.ANSB1
#define pinModeA0 TRISBbits.TRISB1
...
void sendCommand(uint8_t command) {
    SendI2C(SSD1306_DEFAULT_ADDRESS, 0x00, &command, 1);
}

void writeGPIO(uint8_t regAddr, uint8_t regData) {
    //
}
...
void drawRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t f, uint8_t r, uint8_t g, uint8_t b) {

}

void drawRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t f, uint16_t color) {
...
void writeBuffer(uint8_t *buffer, int count) {
    ...
    if (_type == 72) {
        ...
        char tmpArray[8];
        for (char packet = 0; packet < 45; packet++) {
            
            for (char packet_byte = 0; packet_byte < 8; ++packet_byte) {
                tmpArray[packet_byte] = buffer[packet * 8 + packet_byte];
            }
            SendI2C(SSD1306_DEFAULT_ADDRESS, 0x40, tmpArray, sizeof (tmpArray));
        }
    }
    if (_type == 96) {
        ...
        sendCommand(0x00);
        sendCommand(0x01);
        for (char packet = 0; packet < 24; packet++) {
            int buffPos = packet * 8;
            SendI2C(SSD1306_DEFAULT_ADDRESS, 0x40, &buffer[buffPos], 8);
        }
    }
    if (_type == 128) {
        sendCommand(SSD1306_COLUMNADDR);
        ...
        sendCommand(0x07);
        char tmpArray[8];
        for (char packet = 0; packet < 64; packet++) {
...
void setBitDepth(uint8_t b) {

}

void setFlip(uint8_t f) {

}

void setMirror(uint8_t m) {
